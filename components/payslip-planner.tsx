...
function calculateWeeklyIncome(schedule: { day: string; start: number; end: number }[], baseRate: number, overtimeThreshold: number) {
  let totalIncome = 0
  let totalHours = 0
...
export function PayslipPlannerComponent() {
  const [schedule, setSchedule] = useState<Record<string, WorkDay>>(
    Object.fromEntries(
      daysOfWeek.map(day => [day, { isWorking: false, startTime: '09:00', endTime: '17:00' }])
    )
  )
  ...
  const summaryData = [
    { variable: `${summaryPeriod} Gross Income`, amount: grossIncome },
    { variable: `${summaryPeriod} Net Income`, amount: netIncome },
    { variable: `${summaryPeriod} Tax Paid`, amount: taxData ? taxData.taxPerPeriod : 0 },
    ...
    <div className="container mx-auto p-4">
      ...
      <Tabs defaultValue="schedule">
        ...
        <TabsContent value="schedule">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <Select value={summaryPeriod} onValueChange={(value: 'Weekly' | 'Fortnightly') => setSummaryPeriod(value)}>
              ...
              <CardContent className="space-y-4">
                {daysOfWeek.map(day => (
                  <div key={day} className="flex items-center space-x-4">
                    ...
                    />
                    <Label htmlFor={`${day}-toggle`} className="w-24">{day}</Label>
          ...
          <Button className="mt-6" onClick={handleCalculate}>Calculate</Button>
        </TabsContent>
                      ...
                      <TableBody>
                        {incomeData.breakdown.map((item, index) => (
                          <TableRow key={index}>
                            ...
                            <TableCell>{(item.hours * (summaryPeriod === 'Weekly' ? 1 : 2)).toFixed(2)}</TableCell>
                            <TableCell>${item.rate.toFixed(2)}</TableCell>
                            <TableCell>${(item.income * (summaryPeriod === 'Weekly' ? 1 : 2)).toFixed(2)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
